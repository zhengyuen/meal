...
<div class="flex flex-col min-h-screen">
    <div class="flex-1 bg-slate-200">
      <p class="text-2xl text-black">我的訂單</p>
      <p class="text-black">全部 {{ Object.keys(order).length }} 筆</p>
      <div class="shadow-md my-3 py-3 px-5 mx-3 rounded-lg bg-white text-2xl text-bold flex items-center" v-for="(item, idx) in order" :key="idx" @click="changePage(`/orders/${item[0].storeId}`)">
        <div>
      <img src="https://life.ntpu.edu.tw/upload/2022092711003130rlm1.png" alt="image" class="h-20 object-cover">
      </div>
      <div>
        <p class="font-bold text-lg pb-2 ml-5 text-black">小木屋鬆餅</p>
