...
<header>
    <div class="flex mt-3 mx-2">
      ...
      <p class="w-full text-center">{{ t('edit_profile')}}</p>
    </div>
  </header>
  ...
  <form action="" class=" text-center mt-5">
    <div>
      ...
      <div>
        <label for="">{{ t('cellphone') }}</label>
        <input v-model.trim="formData.phone" type="text" class="border-2 border-gray rounded-lg mt-3 ml-2 pl-2 text-black" id="phone">
      </div>
    </div>
  </form>
  <div class="text-center">
    <button class="mt-5 bg-orange-400 rounded-full w-32 text-center disabled:bg-orange-200" @click="submit" :disabled="!formData.name || !formData.user || !formData.phone"  type="submit">{{ t('check') }}</button>
  </div>
